...
def executeAPICall (action : APIAction) (server : Server) : IO (Bool × Option Nat) := do
  ...
  IO.sleep 100 -- (action.endpoint.responseTimeout / 2).toUInt64
...
def traverseDFA (dfa : DFA) (actions : List APIAction) (server : Server) (maxSteps : Nat) : IO ExecutionProof := do
  ...
  -- DFA traversal loop with step limit
  let rec traverse : IO (Bool × List ExecutionStep) := do
    let ts ← stateRef.get

    -- Check termination conditions
    if ts.steps >= maxSteps then
  ...
  -- Execute traversal
  let (finalStateReached, trace) ← traverse
  let ts ← stateRef.get
  let endTime ← IO.monoMsNow
        ...
        | some nextState =>
            ...
            | some action =>
                ...
                if !success then
                  ...
                  | some newServer =>
                      -- Update current server and retry same state (don't increment step count)
